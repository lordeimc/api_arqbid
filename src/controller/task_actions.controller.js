...
ActionActionsCtrl.createAction = async (req, res) => {
    ...
    await pool.query('INSERT INTO `veyron_arqbid`.`t_project_tasks_action` SET ?', [newAction])
...
ActionActionsCtrl.editAction = async (req, res) => {
    ...
    await pool.query('UPDATE `veyron_arqbid`.`t_project_tasks_action` SET ? WHERE `id_action` = ?;', [editedAction, id])
    .then (function (result) {
...
ActionActionsCtrl.deleteAction = async (req, res) => {
    const { id } = req.params;
    await pool.query("DELETE FROM `veyron_arqbid`.`t_project_tasks_action` WHERE (`id_action` = '?');", [id])
    .then (function (result) {
