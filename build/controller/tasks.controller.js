...
taskCtrl.editTask = async (req, res) => {
  ...
  res.send('message: Task Updated... '); //console.log(req.body);
};
...
taskCtrl.deleteTask = async (req, res) => {
  ...
  await pool.query("UPDATE `veyron_arqbid`.`t_Tasks` SET `Task_active` = '0' WHERE `id_Task` = ?;", [id]); //console.log('Product Updated; '+id);
};
/* 
